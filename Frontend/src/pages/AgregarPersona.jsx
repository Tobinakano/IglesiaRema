...
export default function AgregarPersona() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nombre_completo.trim() || !String(formData.numero).trim() || !formData.sexo) {
      setMensajeConfirmacion('Por favor completa todos los campos');
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Guardar el ID de la persona para marcarla como asistida en Asistencias
        localStorage.setItem('personaAcabaDeAgregarse', JSON.stringify({
          id: nuevaPersona.id,
          nombre: nuevaPersona.nombre_completo
        }));
        
        // Redirigir inmediatamente a Asistencias
        navigate('/asistencia/listado');
      } else {
        const error = await res.json();
        setMensajeConfirmacion(error.error || 'Error al agregar la persona');
        setModalConfirmacion(true);
      }
    } catch (error) {
  ...
  if (!session) return <div className="asistencias-loading">Cargando...</div>;
      ...
      <main className="main">
        <div className="page-header">
          <div className="page-header-left">
            <h1 className="page-title">Agregar Persona</h1>
