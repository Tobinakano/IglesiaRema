...
export default function HerramientasListado() {
  ...
  const cargarRegistros = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/asistencia/herramientas/registros`, { credentials: 'include' });
      if (res.ok) {
  ...
  const verListado = async (fechaSeleccionada) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/asistencia/herramientas/registros/${fechaSeleccionada}`, { credentials: 'include' });
      if (res.ok) {
        const data = await res.json();
  ...
  const generarPDF = (fecha, personas) => {
    ...
    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    doc.text('Herramientas para el Camino', 105, yPos, { align: 'center' });
    yPos += 6;
    ...
    grupos.forEach((grupo) => {
      ...
      const [r, g, b] = coloresGrupo[grupo] || [128, 128, 128];
    ...
    doc.save(`Herramientas_${fecha}.pdf`);
  };
  ...
  const confirmaBorrar = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/asistencia/herramientas/registros/${registroBorrar.fecha}`, {
        method: 'DELETE',
        credentials: 'include'
      });
    ...
    <div className="layout">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <a href="/asistencia/herramientas/registros" className="nav-item active">
            <i className="fas fa-clipboard-list"></i>
            Listado de Registros
          </a>
        </nav>
      ...
      <main className="main">
        <div className="page-header">
          <div className="page-header-left">
            <h1 className="page-title">Registros: Herramientas para el Camino</h1>
            <p className="page-subtitle">Historial de asistencias del curso</p>
          </div>
        </div>
                      ...
                      <tr key={registro.fecha} style={{ borderBottom: '1px solid #f0f1f3' }}>
                        ...
                        <td style={{ padding: '16px 20px', textAlign: 'right', display: 'flex', gap: '8px', justifyContent: 'flex-end', alignItems: 'center' }}>
                          ...
                          <button
                            onClick={async () => {
                              try {
                                const res = await fetch(`${API_BASE_URL}/api/asistencia/herramientas/registros/${registro.fecha}`, { credentials: 'include' });
                                if (res.ok) {
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: '12px', padding: '32px', maxWidth: '400px', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.2)' }}>
            ...
            <p style={{ margin: '0 0 24px 0', fontSize: '14px', color: '#555b66' }}>
              ¿Estás seguro de que deseas borrar el registro de Herramientas del <strong>{new Date(registroBorrar?.fecha + 'T00:00:00').toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</strong>?
            </p>
